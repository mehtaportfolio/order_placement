...
function SingleOrderComponent({ backendBase = '', setStatus }, ref) {
  ...
  const [stockRestricted, setStockRestricted] = useState(false)
  const [stockRestrictionMessage, setStockRestrictionMessage] = useState('')
...
const fetchLivePrice = async (symbol) => {
  ...
  const checkStockSurveillance = async (stockName) => {
    const normalized = String(stockName || '').trim()
    if (!normalized) {
      setStockRestricted(false)
      setStockRestrictionMessage('')
      return
    }
    ...
    try {
      ...
      if (res.ok && data?.restricted) {
        setStockRestricted(true)
        setStockRestrictionMessage(data.message || `Market orders are blocked for ${normalized}. Please place a limit order.`)
        setOrderType('LIMIT')
      } else {
        setStockRestricted(false)
        setStockRestrictionMessage('')
  ...
  const handlePlaceOrder = async () => {
    ...
    if (orderType === 'MARKET') {
      try {
        const res = await fetch(`${backendBase}/api/surveillance/check/${encodeURIComponent(stock.trim())}`)
        const data = await res.json().catch(() => null)

        if (res.ok && data?.restricted) {
          setStatus({ type: 'error', message: data.message || `Please place a limit order for ${stock.trim()} because it is in the Trade-to-Trade category.` })
          return
        }
      } catch (err) {
        console.error('Surveillance pre-check failed:', err)
      }
    }
                  ...
                  {stockSuggestions.map((item, index) => (
                    <button
                      ...
                      onMouseDown={async () => {
                        ...
                        setLivePrice(null)
                        await checkStockSurveillance(item.name)
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (stockRestricted && nextType === 'MARKET') {
                setStatus({ type: 'error', message: stockRestrictionMessage || `Market orders are blocked for ${stock.trim()}. Please place a limit order.` })
                setOrderType('LIMIT')
                return
              }
          ...
          {stockRestricted && (
            <div className="hint-box" style={{ marginTop: 8, background: '#fde68a', color: '#92400e' }}>
              {stockRestrictionMessage || `Market orders are blocked for ${stock.trim()}. Please place a limit order.`}
            </div>
          )}
        </div>
      </div>
