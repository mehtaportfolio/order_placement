...
function MultiOrderTab({ backendBase = '', setStatus }) {
  ...
  const handleSelectStock = async (stockName) => {
    setStock(stockName)
    setSearchTerm('')
    await checkStockSurveillance(stockName)
  }
  ...
  const handleAddOrder = async () => {
    ...
    if (orderType === 'MARKET' && stock.trim()) {
      try {
        const res = await fetch(`${backendBase}/api/surveillance/check/${encodeURIComponent(stock.trim())}`)
        const data = await res.json().catch(() => null)
        if (res.ok && data?.restricted) {
          setStatusLocal({ type: 'error', message: data.message || `Please place a limit order for ${stock.trim()} because it is restricted.` })
          return
        }
      } catch (err) {
        ...
        <>
          ...
          <div className="row grid-2">
            ...
            <div>
              ...
              {stockRestricted && (
                <div className="hint" style={{ marginTop: '6px', color: '#f87171' }}>
                  {stockRestrictionMessage}
                </div>
              )}
            </div>
          </div>
