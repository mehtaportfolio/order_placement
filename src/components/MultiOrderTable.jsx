...
function MultiOrderTable({ orders, onUpdate, onDelete, onDuplicate, backendBase = '' }) {
  ...
  const [editingStockRestricted, setEditingStockRestricted] = useState(false)
  const [editingStockRestrictionMessage, setEditingStockRestrictionMessage] = useState('')
...
useEffect(() => {
  ...
  const openEditModal = (order) => {
    ...
    checkEditingOrderStockSurveillance(order.stock)
  }

  const checkEditingOrderStockSurveillance = async (stockName) => {
    const normalized = String(stockName || '').trim()
    if (!normalized) {
      setEditingStockRestricted(false)
      setEditingStockRestrictionMessage('')
      return
    }
    ...
    try {
      ...
      if (res.ok && data?.restricted) {
        setEditingStockRestricted(true)
        setEditingStockRestrictionMessage(data.message || `Market orders are blocked for ${normalized}. Please place a limit order.`)
        setEditingOrder((prev) => (prev ? { ...prev, orderType: 'LIMIT' } : prev))
      } else {
        setEditingStockRestricted(false)
        setEditingStockRestrictionMessage('')
      }
    } catch (err) {
      console.error('Editing stock surveillance check failed:', err)
      setEditingStockRestricted(false)
      setEditingStockRestrictionMessage('')
    }
  }
          ...
          >
            <thead>
              ...
              >
                ...
                <th style={{ padding: '10px', textAlign: 'left', fontWeight: '600', color: '#cbd5e1' }}>Buy Price</th>
                <th style={{ padding: '10px', textAlign: 'left', fontWeight: '600', color: '#cbd5e1' }}>Limit Price</th>
            ...
            <tbody>
              {orders.map((order) => (
                ...
                >
                  ...
                  <td style={{ padding: '10px' }}>{order.buyPrice != null ? `₹${order.buyPrice}` : '-'}</td>
                  <td style={{ padding: '10px' }}>
                    {order.orderType === 'LIMIT' ? (order.price != null ? `₹${order.price}` : '-') : '-'}
                  </td>
                  <td style={{ padding: '10px', fontWeight: '600', color: '#10b981' }}>
                    {ltpMap[order.stock] ? `₹${ltpMap[order.stock].toFixed(2)}` : '--'}
                  </td>
                  <td
                    style={{
                      padding: '10px',
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEditingOrder({ ...editingOrder, stock: nextStock })
                      setSearchTerm(nextStock)
                      setEditingStockRestricted(false)
                      setEditingStockRestrictionMessage('')
                    }}
                    onBlur={() => {
                      if (editingOrder?.stock?.trim()) {
                        checkEditingOrderStockSurveillance(editingOrder.stock)
                      }
                    }}
                    autoComplete="off"
                  />
                      ...
                      {suggestions.map((item, idx) => (
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setSearchTerm('')
                            await checkEditingOrderStockSurveillance(item.name)
                          }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={editingStockRestricted}
                  >
                    <option value="LIMIT">Limit</option>
                    <option value="MARKET">Market</option>
                  </select>
                  {editingStockRestricted && (
                    <div style={{ marginTop: '6px', color: '#f87171', fontSize: '12px' }}>
                      {editingStockRestrictionMessage}
                    </div>
                  )}
                </div>
              </div>
