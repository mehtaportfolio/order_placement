  ...
  return (
    <div className="card">
            ...
            onClick={async () => {
              try {
                ...
                if (res.ok && data.inserted > 0) {
                  ...
                  const message = `All ${data.duplicates} position${data.duplicates === 1 ? '' : 's'} already exist`
                  setStatus && setStatus({ type: 'info', message });
                  setSaveResultMessage(message)
                  setShowSaveResult(true)
                } else if (res.ok && data.inserted === 0) {
                  const message = 'No new positions to save'
                  setStatus && setStatus({ type: 'info', message });
                  setSaveResultMessage(message)
                  setShowSaveResult(true)
                } else {
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (res.ok) {
                  ...
                  const fullMessage = `${data.message || 'Sync complete.'} ${summary ? `| ${summary}` : ''} | ${saveMessage}`;
                  setStatus && setStatus({ type: 'success', message: fullMessage });
                  setSaveResultMessage(fullMessage)
                  setShowSaveResult(true)
                  loadPositionsFull();
                } else {
      ...
      <div className="summary-grid">
        ...
        <div className="summary-card summary-card-pl">
          <div className="summary-label">Total P/L</div>
          <div className={`summary-value ${totalPnlClass}`}>{formatCurrency(totalPnl)}</div>
          <div className={`summary-value-pct ${totalPnlPctClass}`}>({totalPnlPercent.toFixed(2)}%)</div>
        </div>
      </div>
      ...
      {showSaveResult && (
        <ConfirmationDialog
          isOpen={showSaveResult}
          title="Save Results"
          message={saveResultMessage || 'Save completed.'}
          confirmLabel="OK"
          cancelLabel="Close"
          onConfirm={() => { setShowSaveResult(false); setSaveResultMessage(null); }}
          onCancel={() => { setShowSaveResult(false); setSaveResultMessage(null); }}
          totalOrders={0}
        />
      )}
    </div>
